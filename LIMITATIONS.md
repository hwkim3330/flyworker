...
여기에 함정이 하나 더 있었습니다. **초파리 정책도 결정적이 아닙니다** — 감각 뉴런
강제 발화가 포아송 과정이라 난수를 씁니다. 난수 정책만 시드를 고정해 놓고
"이 표는 재현된다"고 쓰고 있었습니다. `Brain.rng` 를 갈아끼울 수 있게 해서
뇌의 난수까지 고정했고, 같은 시드로 두 번 돌려 조향값·최종 위치·누적
스파이크(5,576,128)까지 완전히 일치하는 것을 확인했습니다.

| 정책 | 회차 | 탐색률(평균) | 범위 | **증상 종류(평균)** | 관측된 종류 |
|---|---|---|---|---|---|
| 초파리 커넥톰 | 3 | **42%** | 35–50% | **2.7종** | STUCK, OOB, NOPROG, DOORLOCK |
| 평활 난수 | 5 | 61% | 57–66% | 3.2종 | 다섯 종 전부 |
| 균등 난수 | 5 | 50% | 46–52% | 3.6종 | STUCK, OOB, DOORLOCK, FREEZE |
| 직진만 | 3 | 5% | 5–5% | 1.0종 | FREEZE |

**초파리가 집니다.** 탐색률 42% 대 61% 로 범위가 전혀 겹치지 않고, 한 판에 닿는 증상
종류도 두 난수 정책보다 적습니다. 이기는 항목이 하나도 없습니다.

회차가 3~5회라 유의성을 주장하지는 않습니다. 범위가 겹치지 않는다는 것만 적습니다.

그리고 여기서 QA 지표에 대한 교훈이 하나 나옵니다.
**"버그 수"는 가짜 지표입니다.** 「직진만」이 95건으로 최다인데 전부 같은 정지 버그 하나입니다.
의미 있는 것은 **탐색률과 증상의 종류**입니다.

### 그러면 커넥톰이 난수 위에 뭘 보태기는 하는가

초파리가 단독으로 지는 것은 쟀습니다. 남는 질문은 하나였습니다 —
커넥톰이 난수 위에 무언가를 보태는가?

이건 의견이 아니라 측정으로 답할 수 있는 질문입니다. 프레임워크가 정책을
갈아끼울 수 있으니까요. 그래서 **짝지은 대조군**을 만들었습니다.
초파리 항만 빼고 난수 항은 그대로 두고, **같은 시드를 줘서 완전히 같은 난수열**을
보게 합니다. 그러면 두 정책의 차이는 커넥톰 하나뿐입니다.

| | 탐색률 | 범위 | 증상 종류 |
|---|---|---|---|
| 섞은 정책 (초파리 + 난수) | 53% | 49–56% | 3.7종 |
| 대조군 (난수만, 같은 열) | **56%** | 52–63% | 3.3종 |
| 차이 | **−3%p** | 범위 겹침 | +0.4종 |

**보태는 게 없습니다.** 탐색률은 오히려 3%p 깎이고, 증상 종류는 0.4종 늘지만
둘 다 회차 간 편차 안입니다. 커넥톰을 얹어도 같은 난수를 혼자 쓰는 것과 구별되지
않습니다.

이걸 "초파리가 다양성을 0.4종 높인다"고 쓸 수도 있었습니다. 쓰지 않습니다.
3회차에 범위가 겹치는 차이는 차이가 아닙니다.
...
| 100% 재현된다 | 앱 안 12/12 완전 일치. 내보낸 리포트에서 꺼낸 재현 파일도 Node 에서 `verify_repro.mjs` 로 완전 일치 |
